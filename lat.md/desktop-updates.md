...
`npm run build:portable` creates an unsigned portable executable on a standard Windows account without requiring Developer Mode or administrator rights.

The dedicated `electron-builder.portable.yml` profile disables executable resource editing and signing, avoiding the legacy `winCodeSign` archive and its privileged symlink extraction. The regular `build:win` release path remains unchanged.
