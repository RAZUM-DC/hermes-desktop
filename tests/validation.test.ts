...
describe("validateChatReadiness", () => {
  ...
  it("uses the chat picker override when the persisted model is empty", async () => {
    writeConfig(
      [
        "model:",
        "  provider: alibaba",
  ...
  it("uses the complete picker identity and falls back only for an empty local pick", async () => {
    writeConfig(
  ...
  it("does not borrow local defaults or credentials for a remote selection", async () => {
    writeConfig("model:\n  provider: openrouter\n  default: local-only\n");
    const { validateChatReadiness } = await freshValidation(TEST_DIR);
    const override = {
