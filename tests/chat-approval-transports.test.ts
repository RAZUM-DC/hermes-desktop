...
const activeConnection = vi.hoisted(() => ({
  config: { mode: "local", ssh: {} } as ConnectionConfig,
}));
...
vi.mock("../src/main/config", () => ({
  ...
  getConnectionConfig: () => activeConnection.config,
...
describe("approval transport safety", () => {
  ...
  it("stops unsupported Runs approvals using the original connection, with no approval POST or replay", async () => {
    const cb = callbacks();
    activeConnection.config = remote;
    await send(cb);
